...
export default function Foco() {
  ...
  const { rutina, rutinas, fetchRutinas, getRutinaById, updateUserHabitPreference } = useRutinas();
  ...
  const fetchTareasAgenda = useCallback(async (rangeStart, rangeEnd) => {
  ...
  const fetchDataStable = useCallback(async () => {
    ...
    try {
      const docs = await fetchTareasAgenda(calendarRange.start, calendarRange.end);
      setTareas(docs);
    } catch (error) {
      showAgendaLoadError(error);
      setTareas([]);
    }
  }, [fetchObjetivos, fetchTareasAgenda, calendarRange.start, calendarRange.end, showAgendaLoadError]);
  ...
  // calendarRangeKey estable; fetchTareasAgenda sin deps de snackbar (evita bucle de recargas)
  }, [calendarRangeKey, fetchTareasAgenda]);
  ...
  const syncRutinaForDate = useCallback(async (date) => {
    ...
    try {
      ...
      // noop: el usuario puede personalizar rutina después
    }
  ...
  }, [resolveAgendaTask, updateWithHistory, enqueueSnackbar, fetchDataStable]);
