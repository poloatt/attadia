...
const ObjetivosStats = ({ Objetivos }) => {
  const stats = {
    pendientes: objetivos.filter(p => p.estado === 'PENDIENTE').length,
    enProgreso: objetivos.filter(p => p.estado === 'EN_PROGRESO').length,
    completados: objetivos.filter(p => p.estado === 'COMPLETADO').length,
    atrasados: objetivos.filter(p => {
